// AI CRM 課程教學網站 - 結構化課程資料 (Single Source of Truth)
  ...
  "day1": {
    ...
    "units": [
      ...
      },
      {
        ...
        "prompt": "請接續 Unit 1 的實作內容。現在我們要進入 Unit 2：Spring MVC、REST API 與 CRM Domain Modeling。\n\n我們將使用 AI 協作方式來輔助開發。請在與 AI 溝通時，參考以下高級協作提示詞大師範本以獲取精準的程式碼：\n\n### 用 AI Agent 建立可獨立運行的示範專案\n【建立專案】\n我有一個 Spring Boot 專案，只有 spring-boot-starter-web 依賴。\n請幫我建立一個簡單的客戶 REST API（資料存在記憶體，不用資料庫）：\n- GET /api/customers → 回傳全部客戶\n- GET /api/customers/{id} → 找不到回傳 404\n- POST /api/customers → 新增客戶，回傳 201\n請加上中文函式級別註解。\n\n【驗證 API】\n專案啟動後（port 8080），請幫我用 PowerShell Invoke-RestMethod 測試上面三個端點是否正常回應。\n\n【排查錯誤】\n執行 mvn spring-boot:run 出現以下錯誤：\n[貼上錯誤訊息]\n請幫我找出原因並修正。\n\n\n\n### 【本章實作要求與詳細步驟】\n1. 理解 Spring MVC 的 DispatcherServlet 請求流程\n2. 掌握 REST API 設計原則：URL 是資源名詞，HTTP 方法是動作動詞\n3. 用 AI Agent 建立可獨立運行的客戶 REST API（不含資料庫）\n4. 理解 Controller / Service 的分工，為後續接入 JPA 做準備\n\n請詳細指導我如何一步步實作上述學習目標，並確保所撰寫的程式碼完全符合 Java 21 與 React 19 的開發規範，包含適當的錯誤處理、全域例外攔截與中文註解。\n請接續進行詳細的環境檢測與專案建構步驟說明，確保本專案能在 Windows PowerShell 7+ 環境下順利編譯與啟動。\n請接續進行詳細的環境檢測與專案建構步驟說明，確保本專案能在 Windows PowerShell 7+ 環境下順利編譯與啟動。",
        "promptMac": "請接續 Unit 1 的實作內容。現在我們要進入 Unit 2：Spring MVC、REST API 與 CRM Domain Modeling。\n\n我們將使用 AI 協作方式來輔助開發。請在與 AI 溝通時，參考以下高級協作提示詞大師範本以獲取精準的程式碼：\n\n### 用 AI Agent 建立可獨立運行的示範專案\n【建立專案】\n我有一個 Spring Boot 專案，只有 spring-boot-starter-web 依賴。\n請幫我建立一個簡單的客戶 REST API（資料存在記憶體，不用資料庫）：\n- GET /api/customers → 回傳全部客戶\n- GET /api/customers/{id} → 找不到回傳 404\n- POST /api/customers → 新增客戶，回傳 201\n請加上中文函式級別註解。\n\n【驗證 API】\n專案啟動後（port 8080），請幫我用 PowerShell Invoke-RestMethod 測試上面三個端點是否正常回應。\n\n【排查錯誤】\n執行 mvn spring-boot:run 出現以下錯誤：\n[貼上錯誤訊息]\n請幫我找出原因並修正。\n\n\n\n### 【本章實作要求與詳細步驟】\n1. 理解 Spring MVC 的 DispatcherServlet 請求流程\n2. 掌握 REST API 設計原則：URL 是資源名詞，HTTP 方法是動作動詞\n3. 用 AI Agent 建立可獨立運行的客戶 REST API（不含資料庫）\n4. 理解 Controller / Service 的分工，為後續接入 JPA 做準備\n\n請詳細指導我如何一步步實作上述學習目標，並確保所撰寫的程式碼完全符合 Java 21 與 React 19 的開發規範，包含適當的錯誤處理、全域例外攔截與中文註解。\n請接續進行詳細的環境檢測與專案建構步驟說明，確保本專案能在 macOS zsh / bash 環境下順利編譯與啟動。\n請接續進行詳細的環境檢測與專案建構步驟說明，確保本專案能在 macOS zsh / bash 環境下順利編譯與啟動。",
        "prompts": [
          {
            "title": "高級 AI 協作提示詞大師範本（Windows 版）",
            "note": "可直接交給 Codex / Gemini 進行輔助開發",
      ...
      },
      {
        ...
        "concepts": [
          ...
          {
            "heading": "Flyway 遷移腳本需同步新增 Audit 欄位",
            "body": "新增繼承的 audit 欄位後，資料庫欄位也要對應更新。若已有 V1 建立 products 資料表，此時需要補一支 V2 遷移腳本新增欄位，不可以修改 V1——Flyway 不允許改動已執行過的腳本。\n\n**V2__add_audit_columns.sql (sql)**\n```sql\n-- 為既有資料表補上 audit 欄位\nALTER TABLE products\n    ADD COLUMN created_at TIMESTAMP NOT NULL DEFAULT NOW(),\n    ADD COLUMN updated_at TIMESTAMP NOT NULL DEFAULT NOW();\n\n-- 若 users 資料表也需要 audit 欄位，在同一支腳本一起處理\nALTER TABLE users\n    ADD COLUMN created_at TIMESTAMP NOT NULL DEFAULT NOW(),\n    ADD COLUMN updated_at TIMESTAMP NOT NULL DEFAULT NOW();\n```"
          },
          {
      ...
      },
      {
        ...
        "prompt": "請接續 Unit 3 的實作內容。現在我們要進入 Unit 4：Spring Security、JWT、OpenAPI 與企業級錯誤處理。\n\n我們將使用 AI 協作方式來輔助開發。請在與 AI 溝通時，參考以下高級協作提示詞大師範本以獲取精準的程式碼：\n\n### AI Agent 提示詞 — 身分驗證與角色授權實作\n請在現有專案中，使用 Spring Security 與 JWT 實作安全防護與登入驗證功能：\n1. 引入安全防護套件（Spring Security）與 JWT 依賴，限制除了登入相關的 API 之外，其餘所有的 API 都需要攜帶 JWT Token 進行驗證才能存取。\n2. 實作一個登入 API（例如 POST /api/auth/login），供使用者傳入帳號密碼進行身分驗證。登入成功後，請在 JWT Token 中寫入使用者的角色（簡單區分「管理員 ADMIN」與「一般用戶 USER」），並將 Token 回傳給前端。\n3. 實作角色權限控制：限制客戶資料刪除 API 必須具備「管理員 ADMIN」角色才能執行，而一般查詢與編輯功能則僅需「用戶 USER」或已登入身分即可。\n4. 保護我們的 API 文件（Swagger UI 網頁與相關端點），設定必須在登入驗證並攜帶 JWT Token 後才能正常瀏覽與測試。\n\n\n\n### 【本章實作要求與詳細步驟】\n1. 理解 OpenAPI 規範與 Swagger UI 的關係\n2. 加入 springdoc-openapi 並確認 Swagger UI 可正常存取\n3. 用標註豐富 Controller 的 API 說明\n4. 設定全域 API 資訊（標題、版本、聯絡方式）\n5. 理解沒有全域例外處理的系統有什麼問題\n6. 建立統一的 ErrorResponse 回應格式\n7. 用 @RestControllerAdvice 集中處理各類例外\n8. 自訂業務例外類別，讓錯誤語意更清楚\n9. 理解 Spring Boot 預設 Log 機制與層級\n10. 用 @Slf4j 寫出結構化、有語意的 Log\n11. 透過 application.yml 設定各套件的 Log 層級\n12. 用 Spring Actuator 動態調整 Log 層級，無需重啟應用\n13. 理解 AOP 解決的問題與核心詞彙\n14. 知道 Spring AOP 用 Proxy 實現，並了解其限制\n15. 能辨識哪些 Spring 功能背後使用了 AOP\n16. 知道在什麼情況下才需要直接撰寫 AOP\n17. 理解 Spring Security 的 Authentication（認證）與 Authorization（授權）核心概念\n18. 學會以 AI 輔助建立 JWT 過濾器並在 Token 內寫入使用者角色（ADMIN/USER）\n19. 實作角色基礎存取控制（RBAC），客戶資料查詢 API 供業務人員使用，客戶資料刪除 API 僅限管理員\n20. 保護 Swagger API 文件，限定僅能透過認證身分存取\n\n請詳細指導我如何一步步實作上述學習目標，並確保所撰寫的程式碼完全符合 Java 21 與 React 19 的開發規範，包含適當的錯誤處理、全域例外攔截與中文註解。",
    ...
    "units": [
      {
        ...
        "prompt": "請接續 Unit 4 的實作內容。現在我們要進入 Unit 5：React CRM 工作台與前後端整合。\n\n我們將使用 AI 協作方式來輔助開發。請在與 AI 溝通時，參考以下高級協作提示詞大師範本以獲取精準的程式碼：\n\n### AI Agent 提示詞 — 建立 React 前端專案\n請幫我建立 CRM 智慧工作台的 React 前端專案：\n1. 在專案根目錄用 Vite 建立 React 19 專案：npx create-vite@latest frontend --template react，並執行 npm install。\n2. 設定 vite.config.js 的 server.proxy，把所有 /api 開頭的請求代理到 http://localhost:8080，解決前後端分離的 CORS 問題。\n3. 建立 App 頁面骨架：包含漸層色 Header（linear-gradient 靛藍到紫色）、毛玻璃卡片容器（backdrop-filter: blur），以及聊天視窗的占位區塊。\n4. CSS 請加上骨架屏（skeleton shimmer）載入動畫，之後聊天室等待 AI 回覆時會用到。\n5. 元件需有中文註解。\n完成後請告訴我如何啟動開發伺服器，以及如何確認 proxy 代理有生效。\n\n\n\n### 【本章實作要求與詳細步驟】\n1. 理解 Node.js 與 NPM 相依性套件管理機制\n2. 學會使用 Vite 初始化 React 19 專案的指令步驟\n3. 掌握 JSX 語法特色與 Functional Component 元件結構\n4. 理解為何需要開發代理 (Vite Proxy) 以免除 CORS 限制\n5. 掌握 uiuxpromax 視覺優化要點（毛玻璃、漸層Header、微動畫與骨架屏）\n\n請詳細指導我如何一步步實作上述學習目標，並確保所撰寫的程式碼完全符合 Java 21 與 React 19 的開發規範，包含適當的錯誤處理、全域例外攔截與中文註解。",
