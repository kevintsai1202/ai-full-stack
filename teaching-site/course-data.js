    ...
    "units": [
      {
        ...
        "prompt": "本章我們要建立整個 AI CRM 專案的開發環境基線：用 winget 安裝 JDK 21、Maven、Git、Node.js 與 Docker Desktop，並建立 backend 與 frontend 的 full-stack 專案骨架。理解原理後，依序使用下方提示詞請 AI Agent 協助完成，再用驗證提示詞核對結果。本章提示詞已把欄位、端點與驗收條件寫進去，請整段複製給 AI Agent，不要只貼前半段。",
        "promptMac": "本章我們要建立整個 AI CRM 專案的開發環境基線：用 Homebrew 安裝 JDK 21、Maven、Git、Node.js 與 Docker Desktop，並建立 backend 與 frontend 的 full-stack 專案骨架。理解原理後，依序使用下方提示詞請 AI Agent 協助完成，再用驗證提示詞核對結果。本章提示詞已把欄位、端點與驗收條件寫進去，請整段複製給 AI Agent，不要只貼前半段。",
        "prompts": [
          { "title": "① 用 AI 把開發環境準備好", "kind": "build", "note": "Java、Maven、Git、Node、Docker 一次裝好，產出 check-env.ps1 自己核對", "text": "我要開始學寫程式，請幫我把電腦準備好：裝好寫 Java 程式需要的工具（Java 21、用來建置專案的 Maven、版本控制工具 Git），再裝前端會用到的 Node.js LTS，以及之後跑資料庫要用的 Docker Desktop。全部設定到「打開一個新的終端機視窗就能直接使用」。我用的是 Windows 11，請用 PowerShell 7 操作（如果是 Mac 請改用對應的做法）。完成後請寫一支 check-env.ps1 放在專案根目錄，逐項印出每個工具的版本讓我一次核對，並告訴我怎麼執行它。" },
          { "title": "② 建立專案的資料夾骨架", "kind": "build", "note": "backend / frontend 兩個資料夾，Spring Boot 4.1.x 空專案帶 /api/health", "text": "請幫我建立這個課程專案的資料夾結構：一個 backend 資料夾放「後端程式」、一個 frontend 資料夾放「之後要做的網頁畫面」，並在根目錄開始做版本控制（.gitignore 要排除 target/、node_modules/ 與 .env）。後端先給我一個最簡單、空的、但能跑起來的 Spring Boot 程式就好：用 Spring Initializr 產生，最新的 Spring Boot 4.1.x、Java 21、Maven、Jar，Group 用 com.example、Artifact 用 tutorial，依賴選 Spring Web、Validation、Spring Data JPA、PostgreSQL Driver、Flyway Migration 這五項（資料庫第三章才會接上，所以先在設定檔把資料庫相關的自動設定排除，讓專案現在就能啟動），並提供一個 GET /api/health 回傳 {\"status\":\"UP\"} 的端點。完成後我要能把這個後端空專案實際啟動起來（port 8080），用瀏覽器或 PowerShell 打 /api/health 確認一切就緒。" },
          { "title": "✅ 驗證 — 環境與骨架就緒", "kind": "verify", "note": "確認工具版本與後端能啟動", "text": "請幫我逐一確認開發環境都就緒：執行 check-env.ps1，檢查 Java 21、Maven（其中的 Java version 也要是 21）、Git、Node.js、Docker 的版本都正確；再啟動後端空專案，用 Invoke-RestMethod http://localhost:8080/api/health 確認回 {\"status\":\"UP\"}。如果有任何一項不對，請直接幫我修好，並把修了什麼告訴我。" },
        ...
        "tasks": [
          {
            "id": "u1-t6",
            "label": "安裝 PowerShell 7 並確認 $PSVersionTable 主版本號 >= 7"
          },
          {
            "id": "u1-t1",
      ...
      },
      {
        ...
        "prompt": "請接續 Unit 1。本章我們設計 CRM 的核心資料：客戶、聯絡人、往來紀錄與生意機會。理解原理後，依序使用下方提示詞請 AI 協助你從簡單的記憶體暫存版暖身，做到一套完整的客戶關係資料，最後用驗證提示詞核對。本章提示詞已把欄位、端點與驗收條件寫進去，請整段複製給 AI Agent，不要只貼前半段。",
      ...
      },
      {
        ...
        "prompt": "請接續 Unit 2。本章我們把資料真正存起來：準備一個正式、重開也不會不見的資料庫，用有版本管理的方式建立資料表，並讓客戶查詢支援多個條件任意組合。理解原理後，依序使用下方提示詞請 AI 協助完成，最後用驗證提示詞確認資料確實存住了。本章提示詞已把欄位、端點與驗收條件寫進去，請整段複製給 AI Agent，不要只貼前半段。",
        "promptMac": "請接續 Unit 2。本章我們把資料真正存起來：準備一個正式、重開也不會不見的資料庫，用有版本管理的方式建立資料表，並讓客戶查詢支援多個條件任意組合。理解原理後，依序使用下方提示詞請 AI 協助完成，最後用驗證提示詞確認資料確實存住了。本章提示詞已把欄位、端點與驗收條件寫進去，請整段複製給 AI Agent，不要只貼前半段。",
        "prompts": [
          { "title": "① 準備一個正式的資料庫", "kind": "build", "note": "資料要存得住，重開也不會不見", "text": "請幫我準備一個正式的資料庫來存這些客戶資料：在專案根目錄建立 docker-compose.yml，用最新的 pgvector/pgvector:pg18 映像（PostgreSQL 18 加上向量擴充，之後做「讓 AI 找出相似內容」會用到），資料庫名稱 learn_spring、帳號 postgres、密碼 password、本機 5432 對應容器 5432，並用具名卷（named volume）設定成「資料不會因為重開而消失」。完成後幫我啟動容器，用 docker ps 確認狀態是 Up。每個設定請加繁體中文說明。" },
          { "title": "② 用有版本管理的方式建立資料表", "kind": "build", "note": "Flyway V1 建表、V2 放課程三家客戶的示範資料", "text": "請把「建立資料表」這件事改成用 Flyway 做版本管理（以後修改資料表結構時才追蹤得到、不會亂掉）。V1__init_schema.sql 建立 customers、contacts、interactions、opportunities 四張表，欄位就用第二章定好的那些，每張表都加上 created_at、updated_at、created_by、updated_by 四個稽核欄位，以及 customer_id 外鍵與常用索引。V2__insert_seed_data.sql 放入課程情境的示範資料：\n- 亞太智能製造 (APIM)：智慧工廠與工業物聯網產業、ACTIVE、合約至 2027-12-31；聯絡人 林志明（採購總監）；生意機會「生產線 AI 預測維護模組」1,200,000、NEGOTIATION、NEW_BUSINESS；近 30 天有 3 筆正面的會議、Email、電話往來。\n- 環球零售巨擘 (GlobalMart)：跨國連鎖量販與電商產業、ACTIVE、合約 2026-09-30 到期；聯絡人 陳美玲（技術經理）；生意機會「智能客服與推薦系統」850,000、PROPOSAL、RENEWAL；往來有一筆未解決的系統效能客訴（SUPPORT_TICKET）與一封提到「預算凍結、正在比較競品」的 Email。\n- 鼎峰金融科技 (ApexFin)：財富管理與數位信貸產業、ACTIVE、合約 2026-05-15 已過期；聯絡人 張大衛（營運副總）；生意機會「信用風控與合規 AI 助理」600,000、QUALIFICATION、RENEWAL；最近一次往來在 60 天以前。\n- 再加一家只有基本資料、沒有聯絡人也沒有往來與生意機會的新客戶，之後用來測「資料不足」的情境。\n完成後啟動時，我要看得到這些資料表有成功建立、示範資料有進去。" },
          { "title": "③ 把資料真正存進資料庫，並支援多條件搜尋", "kind": "build", "note": "Specification 多條件任意組合＋固定的分頁格式", "text": "請把剛才暫存在程式裡的客戶資料，改成真正存進資料庫（Customer 等四個類別改成 JPA Entity，加 AuditableEntity 共用稽核欄位，Service 改注入 Repository，Controller 與 DTO 不變）。另外客戶查詢要能「多個條件任意組合」，用 Spring Data JPA 的 Specification 實作：條件有 keyword（公司名或 email 模糊比對）、industry、owner（負責業務）、status、renewalFrom / renewalTo（預計續約日區間），每個條件都可以不填；要有分頁 page（預設 0）與 size（預設 10），回傳格式固定為 items、page、size、totalElements、totalPages。請加繁體中文註解。" },
          { "title": "✅ 驗證 — 資料真的進資料庫、重開也還在", "kind": "verify", "note": "確認持久化且沒有重複建表", "text": "請幫我確認資料確實存進了資料庫：新增一筆客戶後查得到；用 industry 加 keyword 兩個條件一起搜尋，結果與分頁欄位都正確；把專案重開後，那筆資料還在；而且重開時沒有發生「重複建表」之類的錯誤（Flyway 不重複套用、ddl-auto 的 validate 通過）。順便把這幾個檢查加進 scripts/test-crm-api.ps1。" }
        ],
        "tasks": [
          {
            "id": "u3-t1",
          ...
          {
            "id": "u3-t4",
            "label": "用 AI Agent 為 1-2 專案加入 JPA 依賴並更新 Customer 等四個類別為 Entity"
          },
          {
            "id": "u3-t5",
    ...
    "units": [
      {
        ...
        "concepts": [
          ...
          },
          {
            ...
            "body": "理解 React 專案結構、Proxy 原理與視覺優化要點後，複製以下提示詞給 AI Agent，從零建立聊天室的前端專案骨架。\n\n**AI Agent 提示詞 (text)**\n```text\n請幫我建立 CRM 智慧工作台的 React 前端專案：\n1. 在專案根目錄用最新版 Vite 建立 React 19 專案：npx create-vite@latest frontend --template react，並執行 npm install，再安裝 react-router-dom 與 axios。\n2. 設定 vite.config.js 的 server.proxy，把所有 /api 開頭的請求代理到 http://localhost:8080，解決前後端分離的 CORS 問題。\n3. 建立 App 頁面骨架：用 react-router 規劃 /login、/dashboard、/customers、/customers/:id、/team、/my-work 路由；一個 AppShell 版面（左側導覽側欄：總覽、客戶、生意機會、AI 助手、團隊分析、我的工作台；右側內容區），包含漸層色 Header（linear-gradient 靛藍到紫色）、毛玻璃卡片容器（backdrop-filter: blur），以及聊天視窗的占位區塊。\n4. CSS 請加上骨架屏（skeleton shimmer）載入動畫，之後清單載入與聊天室等待 AI 回覆時會用到。\n5. 元件需有繁體中文註解。\n完成後請告訴我如何啟動開發伺服器，以及如何確認 proxy 代理有生效。\n```"
          },
          {
        ...
        "prompt": "請接續 Unit 4。本章我們做前端：建立 CRM 工作台的網頁畫面、串接後端並記住登入，完成登入、客戶清單、詳情與生意機會看板，並把總覽儀表板做成含多張分析圖表與客戶分群的經營儀表板。理解原理後，依序使用下方提示詞請 AI 協助完成，最後做端到端驗證。本章提示詞已把欄位、端點與驗收條件寫進去，請整段複製給 AI Agent，不要只貼前半段。",
        "promptMac": "請接續 Unit 4。本章我們做前端：建立 CRM 工作台的網頁畫面、串接後端並記住登入，完成登入、客戶清單、詳情與生意機會看板，並把總覽儀表板做成含多張分析圖表與客戶分群的經營儀表板。理解原理後，依序使用下方提示詞請 AI 協助完成，最後做端到端驗證。本章提示詞已把欄位、端點與驗收條件寫進去，請整段複製給 AI Agent，不要只貼前半段。",
        "prompts": [
      ...
      },
      {
        ...
        "concepts": [
          ...
          },
          {
            ...
            "group": "Tool Calling 工具呼叫",
            "body": "理解工具呼叫原理與兩種宣告方式後，複製以下提示詞給 AI Agent，讓它把 Day 1 寫好的 Service 包裝成 AI 可呼叫的工具，AI 就能查到資料庫的即時資料。\n\n**AI Agent 提示詞 (text)**\n```text\n請為 CRM AI 助手加入工具呼叫（Tool Calling）能力，讓 AI 能查詢資料庫的即時資料：\n1. 建立 CustomerTools 類別，用 @Tool 註解包裝既有的 CustomerService：\n   - getCustomers(query)：查詢客戶列表（id、name、industry、status、riskLevel），支援關鍵字搜尋，query 為空時回傳全部客戶\n   - getCustomerDetail(id)：依客戶 ID 查詢詳細資訊（產業、狀態、負責業務、合約與續約日、風險等級、最近 5 筆往來紀錄摘要）\n2. 建立 OpportunityTools 類別，用 @Tool 包裝既有的 OpportunityService：\n   - getOpportunities(customerId)：查詢指定客戶的所有生意機會（name、stage、amount、expectedCloseDate、type）\n   - getDealValue(opportunityId)：查詢特定生意機會的金額、階段與預計成交日\n3. 每個 @Tool 的 description 請用中文清楚描述「什麼情況下該呼叫這個工具」，這段文字是寫給大模型看的。\n4. 工具只讀不寫；要套用登入者的權限（SALES 只查得到自己負責的客戶）；回傳給模型前把 email、電話、統編遮罩成 [已遮罩]；查無資料時回傳明確的「查無此客戶」訊息，讓 AI 老實回答而不是編造。\n5. 在 ChatController 的 ChatClient 呼叫鏈中，用 .tools() 掛上這兩個工具類別。\n完成後請示範一句測試提問（例如「亞太智能製造目前的風險等級是什麼？有哪些進行中的生意機會、金額多少？」），並說明如何從後端日誌確認工具真的被呼叫。\n```"
          },
          {
            "heading": "工具呼叫的架構定位",
            "group": "Tool Calling 工具呼叫",
          ...
          },
          {
            ...
            "group": "系統運行展示",
            "body": "使用者詢問特定客戶的銷售進度時（例如「亞太智能製造有哪些進行中的生意機會？」），AI 精確識別客戶名稱並呼叫 OpportunityTools 商機查詢工具，回傳對應的銷售機會。前端會將其渲染出包含商機名稱、金額、成交機率與銷售階段的卡片。"
          },
          {
            "heading": "系統運行展示：長期記憶語意客戶推薦",
            "group": "系統運行展示",
            "body": "當使用者點擊「長期記憶語意推薦」時，後端啟動雙路 RAG 檢索，同時在 pgvector 中查找此使用者在過去幾天內聊過的客戶偏好與銷售重點（例如曾提及專注於製造業、偏好金額大的案子、對特定合作案感興趣等）。AI 結合記憶推薦 CRM 中的特定客戶或機會，前端渲染出極具說服力的語意推薦卡片與推薦理由。"
          }
        ],
        "prompt": "請接續 Unit 5。本章我們讓 CRM 長出 AI 大腦：加上會聊天、會查真實資料的 AI 助手，做一鍵客戶評估報告，並讓系統自動算出客戶風險等級、判讀每筆往來的情緒與意圖。理解原理後，依序使用下方提示詞請 AI 協助完成，最後驗證 AI 回答的數字確實來自真實資料。本章提示詞已把欄位、端點與驗收條件寫進去，請整段複製給 AI Agent，不要只貼前半段。",
        "promptMac": "請接續 Unit 5。本章我們讓 CRM 長出 AI 大腦：加上會聊天、會查真實資料的 AI 助手，做一鍵客戶評估報告，並讓系統自動算出客戶風險等級、判讀每筆往來的情緒與意圖。理解原理後，依序使用下方提示詞請 AI 協助完成，最後驗證 AI 回答的數字確實來自真實資料。本章提示詞已把欄位、端點與驗收條件寫進去，請整段複製給 AI Agent，不要只貼前半段。",
        "prompts": [
          { "title": "① 加上一個會聊天的 AI 智慧助手", "kind": "build", "note": "Groq 端點、SSE 逐字串流、sessionId 記憶、每次呼叫記 ai_call_log", "text": "請幫這套系統加上一個 AI 智慧助手（Spring AI 2.0.x 的 ChatClient），讓我可以用「聊天」的方式問跟客戶有關的問題。模型端點走 Groq 的 OpenAI 相容 API（base-url https://api.groq.com/openai，模型 openai/gpt-oss-120b），金鑰從環境變數 GROQ_API_KEY 讀取，不要寫死在程式碼中。系統提示詞：「你是一個親切的 CRM 智慧助手，只根據系統提供的資料回答，資料不足就明說，不要編造金額或日期」。回答要像打字一樣一個字一個字即時跳出來（SSE 串流，端點 GET /api/ai/stream），而且不同使用者的對話要各自分開記住、不會混在一起（用 MessageChatMemoryAdvisor，記憶 key 用 sessionId，並以登入者身分綁定 sessionId，別人不能用別人的）。每一次呼叫都要記一筆 ai_call_log（呼叫類型、客戶、模型、token 用量、回答摘要、時間），之後第七章要拿來查歷史。請加繁體中文註解。" },
          { "title": "② 讓 AI 回答前先查真實資料、不要亂編", "kind": "build", "note": "Tool Calling 包既有 Service，只讀、套權限、遮罩個資", "text": "請讓這個 AI 助手在回答前，會先去查資料庫裡的真實資料，而不是自己想像：用 Spring AI 的 Tool Calling，把既有 Service 包成工具給模型用——CustomerTools（依關鍵字查客戶清單、依 id 查客戶詳情含風險等級）與 OpportunityTools（查某客戶的生意機會、查某筆生意的金額與階段），每個工具的 description 用中文寫清楚「什麼情況該呼叫」；工具只讀不寫，而且要套用登入者的權限（SALES 只能查到自己負責的客戶）。送給模型的資料要先把 email、電話、統編遮罩。比如我問「環球零售巨擘的生意進度如何」，它要先去撈出那個客戶真正的資料和數字，再根據真實資料回答；問一個不存在的客戶，要回「查無此客戶」而不是編故事。請加繁體中文註解，並告訴我怎麼從後端日誌看到工具真的被呼叫。" },
          { "title": "③ 在網頁上做出 AI 聊天室", "kind": "build", "note": "即時打字效果，而且要確認是已登入的人", "text": "請在網頁上做一個 AI 聊天室（放在客戶詳情頁的側欄，也能從導覽列打開），連上剛才的助手，要有訊息即時一個字一個字跳出來的效果：前端用 EventSource 連 /api/ai/stream?message=…&sessionId=…&token=…，把 localStorage 的 JWT 用網址參數帶上；後端的 JwtAuthenticationFilter 除了 Authorization 標頭，也要能從 token 這個網址參數驗證，確保只有「已經登入的人」才能使用這個聊天，外人不能亂用。畫面要有送出中的骨架屏、錯誤提示、清除對話（清除時換一個新的 sessionId）。請加繁體中文註解。" },
          { "title": "④ 一鍵產生「客戶 AI 評估報告」", "kind": "build", "note": "結訓必做；報告數字一定要來自真實資料", "text": "請加一個「客戶 AI 評估」功能（GET /api/ai/customers/{id}/assessment，同時提供一般回應與 SSE 串流版）：我選一個客戶，後端先把他的基本資料、最近的往來紀錄、進行中的生意、程式算好的風險等級整理成事實清單交給模型，AI 依固定四段輸出 Markdown 報告：「健康度總評」、「生意機會分析」、「風險與預警」、「建議的下一步行動」。報告裡出現的數字一定要來自事實清單，不可以自己編造；報告要存成一筆 ai_call_log，並把報告呈現在客戶詳情頁裡（一個按鈕產生、彈出視窗顯示，可看上一次的報告）。請加繁體中文註解。" },
          { "title": "⑤ 自動算出每個客戶的風險等級", "kind": "build", "note": "60 / 30 天規則由程式算，存欄位、每日重算、可篩選", "text": "請幫每一個客戶自動算出一個「風險等級」（HIGH 高 / MEDIUM 中 / LOW 低），規則由程式算、不要交給 AI 猜：距離最近一次往來超過 60 天、或預計續約日已經過了 → HIGH；超過 30 天 → MEDIUM；完全沒有往來紀錄 → MEDIUM；其餘 → LOW。結果存在客戶的 risk_level 與 risk_computed_at 欄位（用 Flyway 新增），每天凌晨重算一次、新增往來紀錄時也即時重算；客戶清單要能用 riskLevel 篩選，清單與詳情頁要用顏色標籤清楚標示；而且前面做的 AI 評估和聊天回答（CustomerTools 的詳情）也要把它考慮進去。請加繁體中文註解。" },
          { "title": "⑥ 自動判讀每筆往來的情緒與意圖", "kind": "build", "note": "固定的情緒與意圖標籤，存 interaction_insights，做情緒總覽", "text": "請讓系統自動判讀每一筆客戶往來紀錄的「情緒」（POSITIVE 正面 / NEUTRAL 中性 / NEGATIVE 負面，附 -100～100 的分數）和「意圖」（ASK_PRICING 詢價、COMPARE_COMPETITOR 比較競品、CHURN_SIGNAL 有想離開的跡象、RENEWAL_INTEREST 想續約、UPSELL_SIGNAL 想加購、COMPLAINT 客訴、OTHER 其他）：新增往來紀錄時就呼叫模型判讀一次，結果存在 interaction_insights 表（用 Flyway 新增），既有的紀錄在啟動時補算。然後在儀表板做一個情緒總覽（GET /api/dashboard/sentiment）：近期負面互動最多的客戶、出現 CHURN_SIGNAL 或 COMPARE_COMPETITOR 的客戶，特別標出最近有流失風險、需要優先關心的客戶。往來紀錄清單上也要顯示每筆的情緒與意圖標籤。請加繁體中文註解。" },
          { "title": "✅ 驗證 — AI 講的數字是真的，洞察有在運作", "kind": "verify", "note": "數字相符、有風險提醒、有情緒判讀", "text": "請幫我驗證幾件事：問「亞太智能製造目前有哪些進行中的生意？金額多少？」，AI 回答的金額（1,200,000）、階段、往來次數要跟資料庫一致，後端日誌要看得到工具被呼叫；問一個資料庫裡不存在的客戶，AI 要回查無資料；用兩個不同帳號各聊各的，記憶不能互相看到；對鼎峰金融科技做 AI 評估，要看到明顯的風險提醒且風險等級是 HIGH；環球零售巨擘那封提到預算凍結的 Email 情緒要是 NEGATIVE、意圖要是 COMPARE_COMPETITOR 或 CHURN_SIGNAL；並確認 ai_call_log 有這幾次呼叫的紀錄。" }
        ],
        "tasks": [
          {
            "id": "u6-t1",
      ...
      },
      {
        ...
        "concepts": [
          ...
          },
          {
            ...
            "group": "對話歷史長期記憶",
            "body": "理解長期記憶的設計原理（非同步寫入、Metadata 隔離、雙路檢索）後，複製以下提示詞給 AI Agent，為聊天室加上「長期記憶」能力。\n\n**AI Agent 提示詞 (text)**\n```text\n請為聊天室加入「長期記憶」功能，把對話歷史向量化存入 pgvector：\n1. 建立 ChatHistoryService，提供 saveToVectorStore(sessionId, username, customerId, prompt, response) 方法：把使用者問句與 AI 回答組成一份 Document，metadata 加上 type=chat_history、sessionId、username（登入者）、customerId（有的話）與建立時間，再寫入 VectorStore；AI 評估報告產生後也用同一個方法存一份（type=ai_advice）。\n2. 修改 SSE 串流介面：用 doOnNext 收集完整回覆內容，並在 doOnComplete 時用 CompletableFuture.runAsync 非同步寫入向量庫，避免阻塞串流回應。\n3. 實作雙路 RAG 檢索方法：使用者提問時同時查兩路 —— 知識文件（filterExpression 過濾 type 為 knowledge_doc）取前 3 筆、該使用者自己的歷史對話與建議（type 為 chat_history 或 ai_advice，且 username 相符）取前 2 筆，合併後作為上下文交給模型；不可以查到別人的歷史。\n4. 新增 GET /api/ai/history/search?q=…&customerId=…（customerId 可省略）：用語意相似度搜尋登入者自己的歷史對話與 AI 建議，回傳片段、相似度、時間與客戶，讓之後能回頭翻出相關內容做分析；前端在「AI 歷史紀錄」視窗加一個搜尋框接它。\n5. 程式碼需有繁體中文函式註解。\n完成後請示範驗證流程：先聊「我專注製造業客戶、偏好金額大的案子」，之後開新對話問「根據我之前說的需求推薦適合的客戶」，確認 AI 記得歷史偏好；再用 /api/ai/history/search 搜「預算凍結」，確認找得到相關的歷史對話。\n```"
          },
          {
        ...
        "prompt": "請接續 Unit 6。本章我們做兩件高價值的事：一是給主管的團隊分析與 AI 診斷、全公司整體評估，以及每位業務專屬的工作台；二是建立可上傳文件、會標出參考來源的 RAG 知識庫，並把歷史對話與建議也納入長期記憶，讓 AI 越用越聰明（MCP 外部工具列為選修）。理解原理後，依序使用下方提示詞請 AI 協助完成，最後驗證團隊分析與知識庫都正常。本章提示詞已把欄位、端點與驗收條件寫進去，請整段複製給 AI Agent，不要只貼前半段。",
        ...
        "prompts": [
          ...
          { "title": "⑥（選修）讓 AI 接上外部工具", "kind": "build", "note": "行有餘力再做；可先用模擬版", "text": "（這題是進階選修，行有餘力再做）請讓 AI 助手能接上一些外部工具：用 Spring AI 的 MCP Client 連一個自己寫的 MCP Server（放在另一個 Spring Boot 專案，port 8081），提供三個工具——幫忙安排行事曆、產生 Email 草稿、匯出生意報表（先用模擬的版本就好，寫到 log 或回傳假資料）。讓 AI 在對話中除了查公司內部資料，也能順手幫我做這些跨系統的小事；會改變外部狀態的動作要先產生草稿讓我確認，不能自動送出。請加繁體中文註解。" },
          { "title": "✅ 驗證 — 團隊分析與知識庫都正常", "kind": "verify", "note": "主管看得到診斷、文件問答會引用來源", "text": "請幫我驗證：用 manager 登入能看到團隊每個業務的表現與 AI 診斷建議，用 sales 登入呼叫團隊分析會被 403 擋下；sales 能在「我的工作台」看到自己該跟進的客戶（鼎峰金融科技要在風險客戶區）；用 admin 上傳一份『客戶服務規範』後問「客戶服務規範是什麼？」，AI 的回答確實有依照文件內容並標出參考來源，問一個文件裡沒有的問題要說找不到；先聊「我專注製造業客戶、偏好金額大的案子」，開一段新對話問「根據我之前說的需求推薦適合的客戶」，AI 還記得偏好；用 /api/ai/history/search 搜「預算凍結」能找到相關的歷史對話。" }
        ],
        "tasks": [
          {
            "id": "u7-t1",
      ...
      },
      {
        ...
        "prompt": "請接續 Unit 7。本章是結訓衝刺：把前面所有產出整合成一套可展示的 AI CRM，建立端到端測試與上線檢查清單，並準備一條龍的展示流程。理解重點後，依序使用下方提示詞請 AI 協助完成整合與準備，最後完整走一次展示流程驗收。本章提示詞已把欄位、端點與驗收條件寫進去，請整段複製給 AI Agent，不要只貼前半段。",
        "promptMac": "請接續 Unit 7。本章是結訓衝刺：把前面所有產出整合成一套可展示的 AI CRM，建立端到端測試與上線檢查清單，並準備一條龍的展示流程。理解重點後，依序使用下方提示詞請 AI 協助完成整合與準備，最後完整走一次展示流程驗收。本章提示詞已把欄位、端點與驗收條件寫進去，請整段複製給 AI Agent，不要只貼前半段。",
        "prompts": [
          { "title": "① 把所有功能整合成可展示的 AI CRM", "kind": "build", "note": "start-crm.ps1 一鍵啟動＋三種典型客戶的 Playwright 端到端測試", "text": "請接續前面所有做好的東西，把後端、網頁畫面、AI 智慧助手、客戶風險與情緒洞察、團隊分析、我的工作台、以及知識庫問答，全部整合成一套可以完整展示的 AI CRM，確認它們能正確一起啟動運作：寫 start-crm.ps1（起 Docker 資料庫、後端、前端）與 stop-crm.ps1，並整理 README（環境變數清單、三個示範帳號、啟動步驟）。並針對三種典型客戶——高價值的活躍客戶（亞太智能製造）、資料還很少的客戶（那家新客戶）、有流失風險的客戶（環球零售巨擘）——用 Playwright 建立從登入開始、走完客戶詳情、AI 對話、評估報告、知識庫問答的端到端測試，資料不足的客戶要驗證 AI 老實說資料不足而不是編造。請加繁體中文註解。" },
          { "title": "② 上線前檢查與 Demo 展示準備", "kind": "build", "note": "檢查清單 + 一條龍展示腳本", "text": "請幫我整理一份上線前的檢查清單，並逐項幫我檢查：安全性（JWT 密鑰只從環境變數讀且長度足夠、金鑰與密碼不在版控裡、CORS 只允許前端網址、Swagger 需登入、登入與 AI 端點有簡單的限流）、效能（客戶清單分頁、儀表板查詢有快取或索引、SSE 逾時處理）、出錯時的處理（統一 ProblemDetail、AI 呼叫失敗有友善訊息、資料庫連不上時健康檢查回報）。再準備一套展示腳本，依序串起最有亮點的流程：登入 → 看客戶清單 → 跟 AI 對話（問亞太智能製造的生意進度）→ 看客戶評估與風險（鼎峰金融科技）→ 看儀表板分析圖表與情緒總覽 → 看團隊分析 → 用知識庫做一次文件問答，每一步寫清楚要點哪裡、預期看到什麼。" },
          { "title": "✅ 驗證 — 完整走一次展示流程", "kind": "verify", "note": "從零跑通即通過結訓驗收", "text": "請陪我從零完整跑一次驗收：執行 start-crm.ps1 啟動服務、用 sales 登入、瀏覽客戶清單與亞太智能製造的詳情、跟 AI 對話並確認它查到的金額 1,200,000 是真的、看鼎峰金融科技的風險等級與評估報告、看儀表板的分析圖表與情緒總覽、換 manager 看團隊分析、再用知識庫做一次文件問答並看到引用來源，最後跑 scripts/test-crm-api.ps1 與 Playwright 端到端測試，全程沒有錯誤、測試全綠就算通過結訓驗收；請把測試輸出與截圖存到 output/ 讓我留存。" }
        ],
        "tasks": [
          {
            "id": "u8-t1",
