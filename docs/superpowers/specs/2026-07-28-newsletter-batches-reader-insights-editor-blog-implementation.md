...
- 正式資料中的 campaign #4 與 #8 已寄送成功，但建立時間早於「寄送時自動發布」功能，因此 `slug`、`published_at` 皆為 `NULL`，被 archive 的公開查詢正確排除。
- V16 migration 以 campaign ID、完整主旨與 `status = 'sent'` 三重條件精準補上 slug，並沿用 `created_at` 作為原始發布時間；同主旨的 cancelled campaign #3、#7 不發布。
- 六篇有效歷史文章各自取得固定 Emoji 與內容相關的預設 hashtag。所謂隨機圖示是一次性分配後持久化，同一篇文章重新整理時不會變動。
