...
| 登入端點的時間側通道 | **未徹底消除**，以節流限制探測吞吐量；列為已知接受風險，見 8.1 |

### 8.1 已知接受風險：`POST /api/admin/login` 的時間側通道

`sendIfAdmin` 對白名單內的 email 會**同步**走一次寄信呼叫，白名單外則直接 return。
兩者回應內容完全相同（一律 `{"accepted": true}`），但**回應時間差達數百毫秒**，
因此時間本身仍會洩漏「這個 email 是不是管理者」——構成可統計的枚舉 oracle，
與本設計在回應內容上的防枚舉努力（3.1）相抵觸。

**目前的緩解**（限制探測吞吐量，不消除訊號本身）：

- `LoginTokenService.isThrottled`：同一 email 3 封／15 分鐘。
- `LoginAbuseGuard`：同一來源 IP 20 次、全站 200 次／15 分鐘，攻擊者輪替 email 也繞不過。
- 兩者共同把攻擊者能取得的計時樣本數，壓到統計上不足以穩定判別的量級。
- `LoginAbuseGuard.tryAcquire` 在白名單內外**兩條路徑上都會執行**（一次刪除 + 兩次 count +
  一次 saveAndFlush，共 4 次 DB 往返），本身已縮小兩條路徑的時間差。
- 白名單是環境變數指定的少數信箱，不是可枚舉的使用者庫。

**為什麼沒有徹底修掉**：真正的修法是把寄信改為非同步（`@Async` 或事件），
讓兩條路徑的回應時間都不含寄信成本。但本專案目前完全沒有 `@EnableAsync`，
引入它是**全應用範圍**的執行緒模型變更；且它會改變寄信失敗的處理語意
（現為請求執行緒內吞例外並記 log，已有測試鎖住），等於用未驗證的機制換掉已驗證的機制。

**待獨立一輪處理**：屆時須先引入具名 executor 並補足失敗路徑測試，
再把兩條路徑**一起**改為非同步——只改一條會製造新的時間差。
