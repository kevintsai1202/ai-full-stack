...
>
> - **人數**是**兩個來源的聯集去重計數**（`ReferralService.stats`）：
>   ① 帳本 `credit_txn` 中 `reason='REFERRAL'` 的 distinct `note`（note 存的就是被邀者
>   email，同時是發獎冪等鍵，在被邀者**點確認信的那一刻**就寫入）；
>   ∪ ② `ReaderRepository.findInviteeEmailsByReferredBy(referrerId)`（`referred_by`
>   指向自己的讀者 email）。兩邊都以 `ReferralService.normalize` 後的 email 去重，
>   所以同一個人被兩邊都涵蓋時只算一次。
> - **點數**仍加總 `credit_txn` 的 REFERRAL delta（帳本是稽核來源，不可用
>   「人數 × 目前獎勵金額」推算——獎勵金額會被調整，推算值會與歷史實付金額不符）。
>
> **為什麼必須是聯集，而不是「只數 `referred_by`」**（這是修正過的一次判斷）：
> 任一來源單獨使用都會在**常態設定下**顯示錯的數字。`referred_by` 只在被邀者
> **首次登入建立帳戶**時寫入（既有帳戶不回填），而電子報訂閱者**絕大多數永遠不會來
> `/r/` 登入**，沒有任何流程會逼他登入。所以只數 `referred_by` 等於在預設設定
> （`referralReward()=100`）下，把「已經付出去的 100 點」對應的那次邀請藏起來——
> 頁面會顯示「還沒有人透過你的連結完成訂閱」，而點數欄位同時顯示 +100。
>
> 四種情境的覆蓋（完整表格與理由見 `ReferralService.stats` 的 javadoc）：
>
> | 情境 | 帳本 | `referred_by` | 計入 |
> |---|---|---|---|
> | 獎勵 > 0、未登入（最常見） | 有 | 無 | 是 |
> | 獎勵 > 0、已登入 | 有 | 有 | 是（去重後算一人） |
> | 獎勵 = 0、已登入 | 無 | 有 | 是 |
> | 獎勵 = 0、未登入 | 無 | 無 | **否（資料上不存在，無法計入）** |
>
> **最後一格是本節唯一仍未覆蓋的情境，且無法用現有資料補**：獎勵關成 0 時
> `rewardFor` 刻意完全不寫帳本（避免占用冪等鍵，日後把獎勵調回 100 時這位被邀者
> 仍拿得到獎勵），而被邀者若又沒來登入，`referred_by` 也不會寫——這次邀請在資料庫裡
> **沒有留下任何痕跡**。要覆蓋它得在 confirm 時無條件寫一筆歸因紀錄（新表或新欄位
> ⇒ 需要 migration），不在本階段範圍。`/r/invite` 與 `/r/rules` 的 0 值文案已把這個
> 條件寫明（見下段），不對它做程式不保證的承諾。
>
> **人數與點數兩個數字會有落差，兩個方向都可能**，這是刻意接受的：獎勵暫停期間
> 人數可能成長而點數不動（表格第 3 列）；而「被邀者早就有 reader 帳戶」的情境
> （發了獎、`referred_by` 不回填）在聯集之後**已由帳本那一邊涵蓋**，不再造成落差
> ——這一點是舊版 spec 寫錯的地方，當時只數 `referred_by` 才會漏掉它。
>
...
> **在本節實作之前（階段 C～D 之間）PREMIUM 怎麼通知訂閱者**：只能手動另寄一封 BASIC
> 通知信（摘要 + 文章連結，受限內容不得貼入）。這是刻意選擇的過渡做法而非缺漏，
> 理由與硬條件見 §11「階段 C 補件」底下的〈過渡操作模式：PREMIUM 發布後手動寄一封
> BASIC 通知信〉。本節的 `foldedHtml` 完成後該做法即淘汰，兩處應一併更新。
...
> （已解鎖的讀者付的是當時的價）。
>
> **已處理（第一輪，純文案）**：`/r/rules` 與 `/r/me` 的文案先改為「進階文章**通常**每篇 N 點，
...
>
> **已處理（第二輪，改資料來源，2026-07-27 定案）**：加上「通常」只是不再說謊，那個數字
> 仍然不是任何一篇的價格——讀者要的是「我大概要花多少」，而那個問題當時的頁面回答不了。
> 兩頁改為顯示**已發布 PREMIUM 文章的實際點數區間**：
>
> - 新增 `CampaignRepository.findPremiumCostRange(tier)`：一次聚合查詢取回
>   `min(credit_cost)` 與 `max(credit_cost)`，WHERE 同時要求 `tier = 'PREMIUM'`、
>   `slug is not null`、`published_at is not null`。**條件與 archive 列表一致是刻意的**：
>   區間只能反映讀者現在真的打得開、真的會被收這個價的文章。已下架的文章
>   （`published_at` 設回 NULL，見 §11「下架／重新上架」）不得計入，否則規則頁會顯示
>   一個站上根本買不到的價格——那正是本節要消除的那類落差。
> - `PremiumCostDisplay`（`reader` 套件，import `newsletter` 是合法方向）是三分支判斷的
>   唯一產生點，兩頁共用：價格不一 →「目前每篇 12–48 點」；價格相同 →「目前每篇 20 點」
>   （不輸出「20–20 點」，那是把 min／max 的實作細節漏給讀者看）；沒有任何已發布
>   PREMIUM 文章 → 退回 `premiumCost()` 的「通常每篇 N 點」。
> - **最後那個分支是全系統唯一合法使用全域預設的顯示位置**：此時沒有任何「實際扣款額」
>   存在可供顯示，而全域預設正是後台建立下一篇 PREMIUM 時會被預填的值
>   （見 §7 的預填說明），確實是讀者接下來最可能遇到的價格。用「通常」而非「目前」
>   標示它是預估不是事實。
>
> 於是本節的硬要求在**顯示層也真正成立**：頁面上的數字與 `UnlockService` 實際扣的
> 是同一個欄位 `campaign.credit_cost`。表格中「`/r/rules`／`/r/me` 的來源是
> `CreditPolicy.premiumCost()`」一列自此僅適用於「站上沒有已發布 PREMIUM 文章」的空站狀態。
> WHERE 條件由 `CampaignPremiumCostRangeTest` 以真實 PostgreSQL 釘住——把
> `published_at is not null` 拿掉，所有 mock 測試都維持綠燈（實測過），
> 「已下架不得計入」這件事只有真資料庫驗得出來。
>
...
> **「文章設定」的 `credit_cost` 預填（2026-07-27 定案）**：分級切到 PREMIUM 且欄位為空時，
> 自動帶入 `GET /api/admin/settings` 的 `credit.premium_cost`。理由是這個欄位對 PREMIUM
> 無論如何都得填（`validateCreditCost` 與 `ck_campaign_premium_cost` 都要求 `> 0`，
> 見 §5.11），預填讓最常見的「就用預設價」不必手動輸入，也讓操作者看得到目前的預設值
> ——那個數字同時是規則頁在空站狀態下顯示的退路值。
>
> **欄位維持必填、可改，且不得覆蓋操作者已輸入的值**：覆蓋一個已輸入的價格是靜默改價，
> 而**發布後沒有改價路徑**（既有語意：已解鎖的讀者付的是當時的價，不該被追溯改價；
> 要改只能下架重發）。hint 文字明說「發布後價格即凍結」，讓操作者知道後果。
> 因為預填是非同步的（惰性 fetch），`await` 前後各檢查一次欄位是否已被填寫，
> 前者擋同步情形、後者擋競態。設定值在頁面生命週期內快取，
> 但 `saveSettings` 成功後失效——否則剛把預設值改成 50，下一篇仍會被預填成舊的 10。
...
>   **（已修：人數改為「帳本 REFERRAL 的 distinct `note`」∪「`referred_by` 指向自己的讀者 email」
>   的聯集去重計數，不再只數任一邊——只數帳本會在獎勵關成 0 時停止成長，只數 `referred_by`
>   會在預設設定下把已發放獎勵對應的邀請藏起來。仍有一格覆蓋不到（獎勵=0 且被邀者未登入，
>   資料上完全沒有痕跡），0 值文案已把條件寫明；詳見 §5.4）**
...
> ③ 兩篇會同時出現在後台歷史列表。這些都是「不做新功能」換來的，可接受。
>
...
3. **邀請上限與人工審核（2026-07-28 已補強）**：V15 起不再接受「邀請無上限」風險。每位推薦人有可調的每日自動核准上限與十分鐘速度門檻；命中時先寫入 `PENDING_REVIEW`，不先發點，待後台人工核准後才發放推薦人、被邀者與里程碑獎勵。完整設計見 [2026-07-28-viral-growth-system-design.md](2026-07-28-viral-growth-system-design.md)。
