...
/**
 * V21 發點唯一約束：驗證 {@code uq_credit_txn_survey_reward} partial unique index
 * 真的存在於 migration 中，且透過 {@code CreditTxnRepository}（Spring 管理）存取時
 * 撞鍵的例外會轉譯成 {@code DataIntegrityViolationException}——這是應用層
 * {@code existsByReaderIdAndSurveyFormKeyAndReason} 冪等檢查的最終防線，
 * 防止併發重複填答造成重複發點。
 *
 * <p>基底類別、@SpringBootTest 屬性設定與 reader 建立 helper 比照
 * {@code PromoCreditTxnConstraintTest}（V19 同型測試，同套件同基底）；
...
@SpringBootTest(properties = {
    ...
    // 順帶驗證 V21 之後 CreditTxn 的 entity 對應仍通得過啟動時的 validate
    "spring.jpa.hibernate.ddl-auto=validate"
})
class SurveyRewardConstraintTest {
    ...
    /** 獨立的資料庫名稱，不與其他測試共用 */
    private static final String TEST_DB = "survey_reward_credit_txn_test";
    ...
    /**
     * 同一讀者同一問卷的第二筆 SURVEY_REWARD 必須被
     * {@code uq_credit_txn_survey_reward} 唯一索引擋下——這是應用層
     * {@code existsByReaderIdAndSurveyFormKeyAndReason} 冪等檢查的最終防線，
     * 防止併發重複填答造成重複發點。
     */
    @Test
    void 同讀者同問卷第二筆SURVEY_REWARD撞唯一約束() throws SQLException {
        long readerId = insertReader("survey-reward-dup@example.com");
        ...
        assertThrows(DataIntegrityViolationException.class,
            ...
            "同讀者同問卷的第二筆 SURVEY_REWARD 必須被唯一索引擋下");
    }
}
