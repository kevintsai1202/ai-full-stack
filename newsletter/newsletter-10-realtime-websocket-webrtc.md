...
![WebRTC 連線三步曲：經信令伺服器交換 offer/answer、STUN 發現公網位址、ICE 候選互換打通 P2P 通道；TURN 為穿牆失敗時的付費中繼](https://springai-media.zeabur.app/newsletter-media/images/6201aae5f72797dae070cf0f99c632a646b5893e5e547e919768701aa989c1ca.png)
...
有人會問：Webhook 不也是「即時推送」嗎？算，但方向完全不同——**Webhook 是伺服器對伺服器**：外部服務（金流、GitHub、LINE）在事件發生時，對「你的後端」發一個 HTTP 請求。瀏覽器沒有公網位址，**接不到 webhook**，所以它不在瀏覽器即時技術的選型清單裡。

正確的理解是「互補鏈」而不是「替代品」：

```text
金流服務 ──webhook──▶ 你的後端 ──SSE / WebSocket──▶ 使用者的瀏覽器
（付款完成）              （更新訂單狀態）           （畫面即時顯示「付款成功」）
```

如果 AI 在「瀏覽器要即時看到更新」的需求裡端出 webhook，那就是搞錯了受眾——webhook 通知得了你的伺服器，通知不了使用者的螢幕。（Webhook 自己的深水區——簽章驗證、重試與冪等——下期就來講。）
...
瀏覽器端的協定地圖到此完整。下次再有人問「即時功能要用什麼」，把這張表丟給他——下期番外加映：伺服器之間的「它來說」，Webhook 深水區。
