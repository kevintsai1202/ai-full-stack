...
照上期的慣例，每一招都附一句**可以直接丟給 AI 的提示詞**——看懂原理，實作交給它。
...
> 🤖 丟給 AI：「幫我的 SSE 事件加上 id，斷線重連時用 Last-Event-ID 補發漏掉的事件。」
...
> 🤖 丟給 AI：「用 SseEmitter 幫我把 Spring AI 的串流回覆做成 SSE 打字機效果。」
...
> 🤖 丟給 AI：「幫我檢查 SSE 的上線設定：Nginx 關閉緩衝、加上心跳、確認 HTTP/2。」
...
> 🤖 丟給 AI：「幫我評估這個功能該用 SSE 還是 WebSocket，說明理由。」
