# 電子報第 011 期範本：Webhook——伺服器之間的即時通訊，與它的深水區

> **用途**：即時通訊系列的第四期（完結篇）。深入 Webhook：反向 API 的觀念、平台整合場景，
...
1. `金流回調掉單、事件重複入帳——Webhook 的坑都在收的那一端`
2. `Webhook 不是「設個 URL」就完事：簽章、重試、冪等一次講清楚`
3. `即時通訊完結篇：伺服器之間怎麼「它來說」`

**推薦**：`金流回調掉單、事件重複入帳——Webhook 的坑都在收的那一端`

## 預覽文字（preheader）

> 提供方只要送出 HTTP 請求，接收方卻要扛簽章、重試、冪等、順序四道關卡——這期把收 webhook 的正確姿勢一次講完。
