...
def _setup(rms_values: list[float]):
    """建立單一 zone、多句的測試情境（句級位準一律用 RMS，dBFS）。"""
    utterances = [Utterance(index=i, start=i * 2.0, end=(i + 1) * 2.0)
                  for i in range(len(rms_values))]
    zones = [Zone(index=0, start=0.0, end=len(rms_values) * 2.0)]
    return utterances, rms_values, zones


def test_utterance_gain_is_clamped_to_max():
    """一句只有 -40 dBFS（例如咳嗽），增益須被限在 +6dB，不得拉到人聲音量。"""
    utterances, rms, zones = _setup([-20.0, -40.0])
    result = compute_utterance_gains(utterances, rms, zones, [0.0], -20.0,
...
def test_adjacent_gain_step_is_limited():
    ...
    utterances, rms, zones = _setup([-20.0, -30.0, -20.0])
    result = compute_utterance_gains(utterances, rms, zones, [0.0], -20.0,
...
def test_quiet_utterance_gets_positive_gain():
    """小聲句的增益方向必須為正。"""
    utterances, rms, zones = _setup([-20.0, -26.0])
    result = compute_utterance_gains(utterances, rms, zones, [0.0], -20.0)
    assert result[1][2] > 0.0


def test_utterance_gain_accounts_for_zone_gain():
    """區級增益已補償的部分不得被句級重複補償——residual 的定義驗證。

    句 RMS -26 dBFS、zone 增益 +4dB → 套用後 -22，句級只需再補 +2。
    """
    utterances, rms, zones = _setup([-26.0])
    result = compute_utterance_gains(utterances, rms, zones, [4.0], -20.0)
    assert abs(result[0][2] - 2.0) < 1e-6
